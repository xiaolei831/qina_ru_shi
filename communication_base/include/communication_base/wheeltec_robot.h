...
//6轴机械臂串口通信帧格式 (参考 ROS1 table_arm/wheeltec_table_arm)
#define FRAME_HEADER_ARM  0xAA       //机械臂帧头
#define FRAME_TAIL_ARM    0xBB       //机械臂帧尾
#define ARM_SEND_DATA_SIZE 16        //机械臂发送数据帧长度 (6关节×2字节 + 帧头 + mode + 校验 + 帧尾)
...
//6轴机械臂向下位机发送数据的结构体
//ARM send data structure for 6-DOF arm (16 bytes)
typedef struct _ARM_SEND_DATA_
{
	uint8_t tx[ARM_SEND_DATA_SIZE];
}ARM_SEND_DATA;
...
class turn_on_robot : public rclcpp::Node
{
	...
	private:
		...
		bool serialReadByte(uint8_t &byte);
		bool serialWriteBytes(const uint8_t *data, size_t size);
		...
		rclcpp::Subscription<trajectory_msgs::msg::JointTrajectory>::SharedPtr arm_trajectory_Sub;
		rclcpp::Subscription<trajectory_msgs::msg::JointTrajectory>::SharedPtr hand_trajectory_Sub;
   rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr AvoidFlag_Sub;
        ...
        rclcpp::Publisher<qing_robot_msgs::msg::UltrasonicArray>::SharedPtr distance_publisher;

		//超声波发布者
		rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr range_a_publisher_;
		rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr range_b_publisher_;
		rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr range_c_publisher_;
		rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr range_d_publisher_;
		rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr range_e_publisher_;
		rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr range_f_publisher_;
		...
		rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr ChargingMode_publisher;
		...
		void RangerAvoidFlag_Callback(const std_msgs::msg::Bool::SharedPtr AvoidFlag);
		//避障处理函数
		geometry_msgs::msg::Twist avoid_obstacle(const geometry_msgs::msg::Twist& twist);
		//回充相关回调函数
    ...
    void Set_LightRgb_Callback(const std_msgs::msg::UInt8MultiArray::SharedPtr msg);
		...
		//6轴机械臂回调与初始化 (参考 ROS1 table_arm)
		void arm_trajectory_Callback(const trajectory_msgs::msg::JointTrajectory::SharedPtr trajectory);
		void hand_trajectory_Callback(const trajectory_msgs::msg::JointTrajectory::SharedPtr trajectory);
		void send_arm_joint_command(const std::vector<std::string> &names, const std::vector<double> &positions);
		void init_joint_states();  //开机发送机械臂初始位置
		unsigned char Arm_Check_Sum(unsigned char Count_Number);  //机械臂帧独立校验
		string car_mode;
        ...
        ARM_SEND_DATA Arm_Send_Data;  //6轴机械臂发送数据结构体 (16字节)
